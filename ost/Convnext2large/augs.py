...
class Transforms:
    ...
    def resize_and_crops(self): 
        tfs = [
            A.Perspective(scale=(0.05, 0.25), keep_size=True, p=0.8),  # 원래 (0.15, 0.25) → 줄임
    ...
    def jitters(self, p=0.4):
        tfs = [
            A.Superpixels(
                p=p,
                p_replace=(0.1, 0.3),           # 기존 0.1~0.4 → **0.05~0.3로 감소 (과도한 왜곡 방지)**
                n_segments=(10, 40),             # 기존 10~40 → **조금 조정, 세분화도 완화**
                max_size=256,
                interpolation=near
            ),
            A.Downscale(
    ...
    def colors(self, p=0.4):
        tfs = [
            ...
            # A.Solarize(threshold_range=(0.4, 0.6), p=p),
            ...
            # A.PlasmaShadow(
            #     p=p,
            #     shadow_intensity_range=(0.1, 0.4),  # 기존 0.1~0.7 → **0.1~0.4로 감소 (밝은 데이터에 맞게)**
            #     roughness=0.6                       # 기존 0.7 → **0.6로 완화**
            # ),
            A.RGBShift(
                p=p,
                r_shift_limit=20,        # 기존 30 → **20**
                g_shift_limit=20,        # 기존 30 → **20**
                b_shift_limit=25         # 기존 30 → **25 (B 우세 반영)**
            ),
            A.RingingOvershoot(
                p=p,
    ...
    def make_suite(self):
        ...
        tfs += self.colors()
        tfs += self.jitters()
        #crop_transforms = self.resize_and_crops()
        #tfs += crop_transforms * 1
        random.shuffle(tfs)
