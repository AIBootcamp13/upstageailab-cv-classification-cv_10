...
class Transforms:
    ...
    def jitters(self, p=0.4):
        tfs = [
            ...
            A.Downscale(p=p, scale_range=(0.1, 0.4), interpolation_pair=ipp),
            A.GaussNoise(p=p, std_range=(0.1, 0.2)),
    ...
    def easy_rotators(self):
        tfs = [ 
            A.Rotate(p=1.0, limit=(-5, 5)),
            A.SafeRotate(p=1.0, limit=(-5, 5), fill=255),
        ]
        return rand_select(tfs)
