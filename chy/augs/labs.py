...
default_trans = {
    ...
    "medical_outpatient_certificate": "외래/진료/통원 확인서",
...
'''
  config & dashboard & safetensor
'''
def load_hydra_config(config_path, config_name, show_cfg=True):
...
def save_model_tensor(exp_name, model_checkpoint):
    model_checkpoint.best_model_path
    best_model_path = model_checkpoint.best_model_path
    print(f"best checkpoint is {best_model_path}")
    
    checkpoint = torch.load(best_model_path, map_location="cpu")
    state_dict = checkpoint["state_dict"]
    filename = f"{exp_name}-{os.path.basename(best_model_path)}.safetensors"
    save_file(state_dict, filename)
    print(f"saved model: {filename}")

class WandbFinishCallback(pl.callbacks.Callback):
...
class OCRProcessor:
    ...
    def draw_ocr(self, image_path, thickness=4, ratio=4.0, font_size=8, show_img=True):
